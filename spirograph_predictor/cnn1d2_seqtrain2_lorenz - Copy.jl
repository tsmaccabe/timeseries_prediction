...
function train!(model, opt, epochs, initial_lr, min_lr, patience, n_chunks_train, n_chunks_test, batch_size)
    ...
    for epoch in 1:epochs
        ...
        for chunk_idx in chunk_indices
            ...
            train_chunk_loader = load_chunk("D:/timeseries_prediction/data/lorenz/train_chunk_$(chunk_idx).jld2", batch_size)
            chunk_loss = 0.0f0
            num_chunk_batches = 0
        ...
        for chunk_idx in 1:n_chunks_test
            test_chunk_loader = load_chunk("D:/timeseries_prediction/data/lorenz/val_chunk_$(chunk_idx).jld2", batch_size)
            chunk_loss = 0.0f0
...
function build_tcn(input_length, output_length, n_channels, num_blocks, kernel_size, dropout_rate)
    model = Chain(
        tcn_block(n_channels, input_length, kernel_size, 1, dropout_rate),
...
kernel_size = 10
dropout_rate = 0.1
model = build_tcn(input_length, output_length, n_channels, n_blocks, kernel_size, dropout_rate) |> gpu

# Define the loss function and optimizer
loss(x, y) = Float64(Flux.mse(model(x), y))# + l2_reg(model, 0.0001)
...
plt1 = plot(train_losses, xlabel="Epoch", ylabel="Loss", label="Training Loss", color = :blue, legend = :topleft)
plot!(plt1, test_losses, label = "Test Loss", color = :green)
display(plt1)

test_chunk_loader = load_chunk("D:/timeseries_prediction/data/lorenz/val_chunk_1.jld2", batch_size)
X1gpu = reshape(first(test_chunk_loader)[1][1][:, :, :, 1], (input_length, 3))
Y1bar = reshape(model(reshape(X1gpu, input_length, 1, 3, 1)), (output_length, 3)) |> cpu

Y1 = reshape(first(test_chunk_loader)[1][2][:, 1], (output_length, 3)) |> cpu
plt2 = plot(Y1, 1:output_length, marker = :+, linewidth = 0, markersize = 1)
plot!(plt2, Y1bar, 1:output_length, marker = :x, linewidth = 0, markersize = 1)
